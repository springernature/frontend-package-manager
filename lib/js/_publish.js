 ...
 * @return {Promise}
 */
async function publishPackage(pathToPackage, rootPath, scope, packageName) {
		...
		try {
			...
			for (const toolkit of Object.keys(toolkitConfig)) {
				...
				for (const pathToPackage of packagePaths) {
					await publishPackage(pathToPackage, rootPath, contextConfig.scope, contextConfig.brandContextName);
				}
			}
