 ...
 * @return {Promise}
 */
async function publishPackage(pathToPackage, scope, packageName) {
	if (await checkCurrentVersion(pathToPackage, true)) {
		await createInstallMessage(pathToPackage, scope, packageName);
...
async function publishContext(config) {
	const pathToPackage = path.resolve(currentWorkingDirectory, config.contextDirectory, config.brandContextName);
...
module.exports = allConfigs => {
	...
	(async () => {
		try {
			...
			await publishContext(contextConfig);
