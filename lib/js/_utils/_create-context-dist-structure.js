...
async function getConfiguredEndpoints(pathToPackage, configuredBrands) {
	...
	const javascriptEndpoints = ['default/js/helpers/index.js']; // glob for all index files in /js/
	...
	await Promise.all([...javascriptEndpoints, ...sassEndpoints].map(async endpoint => {
		...
		try {
			...
			distributionConfig[assetType].push({
				...
				...(assetType === 'css' && {destination: path.join(distParentFolder, 'css', brand, `${path.parse(endpoint).name}.css`)}),
				...(assetType === 'js' && {destination: path.join(distParentFolder, 'js', brand, endpoint.split('/js/')[1])})
			});
		} catch {
