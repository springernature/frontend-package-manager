/**
 * _create-package-dist-structure.js
...
function getToolkitName(pathToPackage, allToolkitPaths) {
	for (const toolkit of Object.keys(allToolkitPaths)) {
		const toolkitPath = allToolkitPaths[toolkit].path;
 ...
 * @async
 * @function createPackageDistStructure
 * @param {String} pathToPackage package path on filesystem
 ...
 */
async function createPackageDistStructure(pathToPackage, config) {
	...
	const jsResult = await globby(`${jsPath}/index.js`, {expandDirectories: false, onlyFiles: true});
...
module.exports = createPackageDistStructure;
