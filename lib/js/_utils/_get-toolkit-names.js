 ...
 */
async function getAllToolkitNames(defaultConfig, currentWorkingDirectory) {
	const toolkitNames = [];

	// Check this is being run in a directory with a toolkits folder
	try {
		await exists.directoryExists(path.join(currentWorkingDirectory, defaultConfig.toolkitsDirectory));
	} catch (error) {
