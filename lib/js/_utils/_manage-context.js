...
async function checkHasToolkitContext(toolkit, contextDependencies) {
	if (!contextDependencies.includes(toolkit)) {
		reporter.fail('missing context', 'must define a context for package toolkit', toolkit);
 ...
 * @return {Promise}
 */
async function validate(allToolkitNames, toolkit, config, packagePath) {
	const packageJsonPath = path.resolve(packagePath, 'package.json');
	const packageJson = require(packageJsonPath);
	const hasContext = Boolean(packageJson.contextDependencies);
	...
	await checkFileNaming(config.enforceBrandFileNaming);

	// Validate context dependencies
	for (const context of Object.keys(packageJson.contextDependencies)) {
		await getContextInfo(config.scope, context, packageJson.contextDependencies[context]);
	}
}
