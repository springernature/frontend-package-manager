/**
 * _manage-context.js
 * Manage the handling of brand context for packages
 */
'use strict';
...
/**
 * Check that brand context has been set up properly
 * @async
 * @function validate
 ...
 * @return {Promise}
 */
async function validate(config, packagePath, brands) {
	const packageJsonPath = path.resolve(packagePath, 'package.json');
	const packageJson = require(packageJsonPath);
	...
	await checkContextVersion(config.scope, config.brandContextName, packageJson.brandContext);
