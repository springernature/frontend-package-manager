...
async function configGenerator(defaultConfig, packageJsonPath, allToolkitsInfo) {
	...
	const toolkitsConfigLocation = path.resolve(process.cwd(), rootPath, toolkitsDirectory, 'package-manager.json');
	const allConfigs = {};
	...
	const sharedConfig = await getRepoConfig(defaultConfig, toolkitsConfigLocation, path.basename(rootPath), toolkitsDirectory);
