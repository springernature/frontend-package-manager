...
async function getToolkitConfig(toolkitConfigLocation, toolkitName) {
	try {
		...
		reporter.success('toolkit config found', toolkitName, `./${path.relative(directoryPath, toolkitConfigLocation)}`);
...
async function getRepoConfig(defaultConfig, repoConfigPath, toolkitRepoName, toolkitsDirectory) {
	try {
		...
		reporter.success(`config found in repository ${toolkitsDirectory} folder`, toolkitRepoName, `./${path.relative(directoryPath, repoConfigPath)}`);
...
async function buildToolkitConfig(toolkitInfo, toolkitName, sharedConfig) {
	...
	const toolkitConfigLocation = path.join(directoryPath, toolkitInfo.path, 'package-manager.json');
	...
	const packagesDirectoryPath = path.join(directoryPath, toolkitInfo.path, mergedConfigs.packagesDirectory);
 ...
 * @return {Promise<Object>}
 */
async function configGenerator(defaultConfig, currentWorkingDirectory, allToolkitsInfo) {
	directoryPath = currentWorkingDirectory;
	const toolkitsDirectory = defaultConfig.toolkitsDirectory;
	const toolkitsConfigLocation = path.join(directoryPath, toolkitsDirectory, 'package-manager.json');
	const allConfigs = {};
	...
	const sharedConfig = await getRepoConfig(defaultConfig, toolkitsConfigLocation, path.basename(directoryPath), toolkitsDirectory);
