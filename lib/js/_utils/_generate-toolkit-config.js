...
async function buildToolkitConfig(toolkitInfo, toolkitName, sharedConfig) {
	...
	const toolkitConfigLocation = path.resolve(currentWorkingDirectory, toolkitInfo.path, 'package-manager.json');
	...
	const packagesDirectoryPath = path.resolve(currentWorkingDirectory, toolkitInfo.path, mergedConfigs.packagesDirectory);
 ...
 * @return {Promise<Object>}
 */
async function configGenerator(defaultConfig, packageJsonPath, allToolkitsInfo) {
	const directoryPath = path.dirname(packageJsonPath);
	const toolkitsDirectory = defaultConfig.toolkitsDirectory;
	const toolkitsConfigLocation = path.resolve(currentWorkingDirectory, directoryPath, toolkitsDirectory, 'package-manager.json');
	const allConfigs = {};
