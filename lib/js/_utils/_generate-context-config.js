...
async function getContextConfig(configLocation) {
	try {
		...
		reporter.success('context config found', null, `./${path.relative(currentWorkingDirectory, configLocation)}`);
...
async function configGenerator(defaultContextConfig) {
	...
	const contextConfigLocation = path.resolve(currentWorkingDirectory, contextDirectory, 'package-manager.json');

	reporter.info('building context configuration');

	// Get config for the context directory within the repo
	const contextConfig = await getContextConfig(contextConfigLocation);
