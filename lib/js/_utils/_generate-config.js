...
async function getToolkitConfig(toolkitConfigLocation, toolkitName) {
	try {
		...
		reporter.success('toolkit config found', toolkitName, toolkitConfigLocation);
...
async function getBaseConfig(baseConfigPath, toolkitRepoName) {
	try {
		...
		reporter.success('config found in repository root', toolkitRepoName, baseConfigPath);
 ...
 * @param {Object} toolkitInfoObject data about all found toolkits
 * @param {String} toolkitName name of this toolkit
 * @param {Object} baseConfig default config options
 * @return {Promise<Object>}
 */
async function buildToolkitConfig(toolkitInfoObject, toolkitName, baseConfig) {
	...
	try {
		await Promise.all(
			Object.keys(toolkitInfoObject).map(async toolkitName => {
				...
				toolkitInfoObject = await buildToolkitConfig(toolkitInfoObject, toolkitName, baseConfig);
			})
		);
	} catch (err) {
		throw err;
