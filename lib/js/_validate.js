...
const validatePackageContext = require('./_utils/_manage-context').validate;
 ...
 * @return {Promise}
 */
async function validatePackage(config, brands, pathToPackage) {
	...
	await validatePackageContext(config, pathToPackage, brands);
...
module.exports = (packageJsonPath, allConfigs, validate) => {
	...
	(async () => {
		try {
			...
			await validateContext(contextConfig.config);
			...
			for (const toolkit of Object.keys(toolkitConfig)) {
				...
				for (const path of packagePaths) {
					await validatePackage(config, contextConfig.config.brands, path);
				}
			}
