 ...
 */
async function createDistStructure(pathToPackage, brands, allToolkitPaths) {
	...
	const toolkitName = getToolkitName(pathToPackage, allToolkitPaths);
	...
	checkValidEndpoints(distCssPaths, 'scss', brands, toolkitName);

	// No assets means no distributable
	if (distJsPaths.length === 0 && distCssPaths.length === 0) {
		reporter.warning('compiling static assets', 'no endpoints found');
		return;
	}
