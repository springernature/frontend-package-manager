...
const installMessageRegistry = 'https://registry.npmjs.com/';
...
async function createInstallMessage(pathToPackage, scope, packageName) {
	...
	const npxCommand = `npm_config_registry=${installMessageRegistry} npx ${installMessagePackage}@${latestInstallMessageVersion} -p ${thePackage} -v ${matchingVersions.join(' ')}`;
