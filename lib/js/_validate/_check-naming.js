 ...
 * @async
 * @function validNaming
 * @param {String} config toolkit config
 * @param {String} pathToPackage package path on filesystem
 * @return {Promise}
 */
async function validNaming(config, pathToPackage) {
	const startsWith = (config.prefix) ? `${config.prefix}-` : '';
	const folderName = getPackageName(pathToPackage);
	const packageName = require(`${pathToPackage}/package.json`).name;
