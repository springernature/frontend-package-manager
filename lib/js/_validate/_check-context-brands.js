...
async function checkContextBrands(config, pathToPackage) {
	const allowedBrands = config.config.brands;
	let contextBrands;

	reporter.info('checking', 'allowed sbrand configurations', allowedBrands.join(', '));
	...
	try {
		...
		if (contextBrands.length === 0) {
			throw new Error(`${config.config.brandContextName} must contain at least ONE brand`);
		}
		...
		reporter.fail('validating', 'context brand configurations', contextBrands.join(', '));
		throw error;
	}
