...
async function checkContextStructure(config) {
	const contextPath = path.resolve(process.cwd(), config.path);
	const contextName = config.config.brandContextName;
	...
	reporter.success('validating', 'brand context structure', `./${config.path}/${contextName}`);
