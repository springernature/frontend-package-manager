	...
	try {
		...
		for (const file of files) {
			...
			if (!brands.includes(filename)) {
				throw new Error(`\`${filename}\` is not a valid brand: ${(brands.length > 0) ? brands.join(', ') : 'no brands defined'}`);
			}
		}
	} catch (error) {
 ...
 * @return {Promise}
 */
async function checkFileNaming(enforceBrandFileNaming, brands, pathToPackage) {
	...
	for (const location of enforceBrandFileNaming) {
		...
		await checkAllFilesInLocation(path.join(pathToPackage, location), location, brands);
	}
