...
function isFolder(filePath, relativeFilePath) {
	...
	if (!isValid) {
		reporter.fail('validating', relativeFilePath, 'is not a valid folder');
...
async function checkTopLevelFiles(packagePath, packageName) {
	try {
		const filePaths = await globby(`${packagePath}/*`, {onlyFiles: true});
		...
		for (const filePath of pathsToValidate) {
			...
			if (isRequired(relativeFilePath)) {
				// Valid required file at the top level
				...
				reporter.fail('validating', relativeFilePath, 'is not a valid top level file');
	...
	try {
		...
		for (const filePath of pathsToValidate) {
			...
			if (isRequired(relativeFilePath)) {
				// Valid required file at the top level
				reporter.success('validating', relativeFilePath, 'required file exists');
			} else if (isFolder(filePath, relativeFilePath)) {
				// Valid folder at the top level
				reporter.success('validating', relativeFilePath, 'is a valid folder');
			} else if (isFileType(relativeFilePath)) {
				// Valid file type within a valid folder
				reporter.success('validating', relativeFilePath, 'is a valid file');
			} else {
				// If not recongnised at any other step then invalid top level file
				reporter.fail('validating', relativeFilePath, 'is not a valid top level file');
				results.topLevel = false;
...
async function init(validationConfig, packagePath, configuredBrands) {
	...
	// Brand context is organised into brand folders
	if (configuredBrands) {
		// Validate top level files in context
		await checkTopLevelFiles(packagePath, getPackageName(packagePath));

		// Validate files within each brand
		for (const brand of configuredBrands) {
			await checkPackageStructure(path.resolve(packagePath, brand), getPackageName(packagePath));
		}
	} else {
