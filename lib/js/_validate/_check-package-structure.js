...
/**
 * Glob files/folders in a package
 * @private
 * @async
	...
	try {
		...
		for (const filePath of pathsToValidate) {
			...
			if (isRequired(relativeFilePath)) {
				...
				reporter.success('validating', relativeFileName, 'required file exists');
			} else if (isFolder(filePath, relativeFilePath, brand)) {
				// Valid folder at the top level
				reporter.success('validating', relativeFileName, 'is a valid folder');
			} else if (isFileType(relativeFilePath, brand)) {
				// Valid file type within a valid folder
				reporter.success('validating', relativeFileName, 'is a valid file');
			} else {
