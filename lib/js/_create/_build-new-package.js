			...
			folderTasks.push({
				...
				task: () => fs.mkdir(`${filePath}/${answers.pkgname}/${value}`, err => {
					if (err) {
						...
						if (value === 'scss') {
							['10-settings', '20-functions', '30-mixins', '40-base', '50-components', '60-utilities'].forEach(subfolder => {
...
function generateFiles(config, answers) {
	for (const value of config.required) {
		if (value === 'package.json') {
			...
			fileTasks.push({
				title: `Create file ${chalk.bold.white(value)}`,
				task: () => fs.writeFile(`${filePath}/${answers.pkgname}/${value}`, '__tests__ coverage **/*.html HISTORY.md', err => {
					if (err) {
						console.error(err);
					}
				})
