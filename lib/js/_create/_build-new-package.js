...
function createCSSDirectoryStructure(subFolders, answers, parentFolder) {
	subFolders.forEach(subFolder => {
		fs.mkdir(`${filePath}/${answers.pkgname}/${parentFolder}/${subFolder}`, err => {
			if (err) {
				reporter.fail(`problem creating '${subFolder}' in CSS directory structure`, null, `${answers.pkgname}/${parentFolder}/${subFolder}`);
				console.error(err);
			}
		});
	});
		...
		for (const folder of answers.folders) {
			folderTasks.push({
				...
				task: () => fs.mkdir(`${filePath}/${answers.pkgname}/${folder}`, err => {
					if (err) {
						reporter.fail('problem creating folder', null, `${filePath}/${answers.pkgname}/${folder}`);
						console.error(err);
					} else {
						fs.writeFile(`${filePath}/${answers.pkgname}/${folder}/.gitkeep`, '// temporary file, delete as appropriate', err => {
							if (err) {
								reporter.fail(`problem creating temporary file in '${folder}'`, null, '.gitkeep');
								console.error(err);
							}
						});
					}
		...
		if (file === 'package.json') {
			fileTasks.push({
				...
				task: () => fs.writeFile(`${filePath}/${answers.pkgname}/${file}`, packageJsonContents, err => {
					if (err) {
						reporter.fail(`problem creating '${file}'`, null, `${answers.pkgname}/${file}`);
						console.error(err);
					}
				})
			});
	...
	return [
		{
			...
			task: () => fs.mkdir(`${filePath}/${answers.pkgname}`, err => {
				if (err) {
					reporter.fail('problem creating folder for the package', null, `${answers.pkgname}`);
					console.error(err);
				}
			})
		},
