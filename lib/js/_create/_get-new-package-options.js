...
function checkValidName(prefix, existingPackages, name) {
	const fullPackageName = prefixName(prefix, name);
	const stripPrefixName = fullPackageName.replace(`${prefix}-`, '');

	if (
		(prefix && fullPackageName === `${prefix}-`) ||
		(!prefix && fullPackageName.length === 0)
	) {
		return `Component is invalid: name is blank`;
	}

	if (stripPrefixName.includes(prefix)) {
		return `Component \`${stripPrefixName}\` is invalid. Must not contain the prefix name`;
	}

	if (checkPackageExists(existingPackages, fullPackageName)) {
		return `Component \`${fullPackageName}\` already exists`;
	}

	if (!validatePkgName(fullPackageName).validForNewPackages) {
		return `Component \`${fullPackageName}\` is not a valid NPM package name`;
	}
